...
class Functions {
	...
	static function renderGameFrame() {
		...
		printf( $iframe_string, $iframe_src, $game_thumb_bg, $game_demo_btn_text, $game_age_gate_text, '' );
	}
	...
	static function renderGameWidget() {
		...
		if ( empty( $single_game_widget ) ) {
			echo wp_kses(
				__( '<p><strong>Widget Area:</strong> Please add widgets via Appearance > Widgets</p>', 'vegashero' ),
				[
					'p'      => [],
					'strong' => [],
				]
