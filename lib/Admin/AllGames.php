...
class AllGames {
	...
	const GAME_TYPES = array( 'html5', 'flash' );

	public function __construct() {
		$this->_config = \VegasHero\Config::getInstance();
		add_filter( sprintf( 'manage_%s_posts_columns', $this->_config->customPostType ), array( $this, 'addGameTypeColumn' ) );
		add_action( sprintf( 'manage_%s_posts_custom_column', $this->_config->customPostType ), array( $this, 'addGameTypeColumnValues' ), 10, 2 );

		add_action( 'restrict_manage_posts', array( $this, 'addGameTypeFilter' ), 10, 1 );
		add_filter( 'parse_query', array( $this, 'filterGamesByGameType' ), 10 );
	}
