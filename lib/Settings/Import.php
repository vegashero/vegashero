...
abstract class Import {
	...
	protected function _getGameTypeCheckboxes( $slug, $html5_total, $flash_total ) {
		...
		$checkboxes .= wp_kses(
			sprintf( '<div class="vh_game_type_checkbox"><input type="checkbox" id="%1$s_html5" name="vh-import-html5" checked><label for="%1$s_html5">%2$s (%3$d)</label></div>', $slug, $html5_text, $html5_total ),
			[
				'div'   => [ 'class' => true ],
				'input' => [
					'type'    => true,
		...
		$checkboxes .= wp_kses(
			sprintf( '<div class="vh_game_type_checkbox"><input type="checkbox" id="%1$s_flash" name="vh-import-flash"><label for="%1$s_flash">%2$s (%3$d)</label></div>', $slug, $flash_text, $flash_total ),
			[
				'div'   => [ 'class' => true ],
				'input' => [
					'type'    => true,
	...
	protected function _fetchList( $endpoint ) {
		...
		if ( empty( $items ) ) {
			...
			if ( ! is_wp_error( $response ) ) {
				// TODO: do something about error scenario
