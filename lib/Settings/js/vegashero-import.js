...
jQuery(document).ready(function($) {
    ...
    GameImporter.importButtons.click(function(event) {
        ...
        let html5 = $(this).parent().siblings(".footer-area").find('input[name="vh-import-html5"]').prop('checked');
        let flash = $(this).parent().siblings(".footer-area").find('input[name="vh-import-flash"]').prop('checked');
        ...
        GameImporter.fetch(url)
            .done(function(games) {
                ...
                GameImporter.footer.children(".spinner").html(notice);
                AdminNotice.triggerLoading(notice, "info");
                GameImporter.batchImport(totalGames, games, GameImporter.button.dataset.import, 0);
            })
            .fail(function(xhr, status, error) {
