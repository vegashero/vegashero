...
class License extends Settings {
	...
	public function activateLicense( $license ) {
		if ( ! empty( $license ) && $license !== get_option( 'vh_license' ) ) {
			...
			$api_params = array(
				...
				'item_name'               => urlencode( static::$_config->eddItemName ),
			...
			$response = wp_remote_post(
				static::$_config->eddStoreUrl,
				array(
					'timeout'   => 15,
					'sslverify' => false,
					'body'      => $api_params,
					'headers'   => [
						'EDD-LICENSE' => trim( $license ) // Cloudflare to allow POST requests to vegashero.co/ with this header
					]
				)
			);
	...
	public function sectionHeading() {
		...
		$id    = $args[0]['id'];
		$title = $args[0]['title'];
		include_once __DIR__ . '/templates/license/section-heading.php';
	}
	...
	public function inputForLicense() {
		...
		include_once __DIR__ . '/templates/license/license-input.php';
	}

	public function licenseStatus() {
		$args = func_get_args();
		$id   = $args[0]['id'];
		include_once __DIR__ . '/templates/license/license-status.php';
	}
