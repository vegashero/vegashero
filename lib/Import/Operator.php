...
class Operator extends Import {
	...
	public function __construct() {
		...
		add_action(
			...
			function () {
				...
				register_rest_route(
					$namespace,
					self::getFetchApiRoute() . '(?P<operator>.+)',
					array(
						'methods'             => 'GET',
				...
				register_rest_route(
					$namespace,
					self::getImportApiRoute() . '(?P<operator>.+)',
					array(
						'methods'             => 'POST',
	...
	private function _insertNewGame( object $game, string $operator, array $properties = [] ) {
		...
		$game_operator_term_id = wp_set_object_terms( $post_id, $operator_id, $this->_config->gameOperatorTaxonomy );
		// link operator and post
