...
final class GamesGrid {
	 ...
	 * @return string
	 */
	public static function render( $attributes, $config ) {
	...
	private static function _getPageQueryParamName(): string {
	 ...
	 * @return string
	 */
	private static function _getPreviousLink( $text, $current_page ) {
		if ( ! self::_isPage() ) {
	 ...
	 * @return string
	 */
	private static function _getPreviousPostLink( $text, $current_page ) {
		$url = add_query_arg(
			array(
	 ...
	 * @return string
	 */
	private static function _getPreviousPageLink( $text ) {
	 ...
	 * @return string
	 */
	private static function _getNextLink( $text, $current_page, $max_num_pages ) {
		if ( self::_isPage() ) {
	 ...
	 * @return string
	 */
	private static function _getNextPostLink( $text, $current_page ) {
		$url = add_query_arg(
			array(
				'paged' => (int) $current_page + 1,
			)
	 ...
	 * @return string
	 */
	private static function _getNextPageLink( $text, $max_num_pages ) {
	 ...
	 * @return string
	 */
	private static function _getPaginationMarkup( $current_page, $max_num_pages ) {
	 ...
	 * @return bool
	 */
	private static function _isFirstPage( $current_page ) {
	 ...
	 * @return bool
	 */
	private static function _isLastPage( $max_num_pages ) {
	 ...
	 * @return string
	 */
	private static function _getGameMarkup( $post, $thumbnail_url ) {
...
MARKUP;
	 ...
	 * @return bool
	 */
	private static function _isPaginated( $query, $pagination ) {
	 ...
	 * @returns object
	 */
	private static function _getAttributes( $attributes = array(), $paged = 1 ) {
	 ...
	 * @return array
	 */
	private static function _getQueryParams( $attributes, $config ) {
		$query_params = [
	 ...
	 * @return string
	 */
	private static function _getThumbnail( $post, $config ) {
		...
		} elseif ( ! $thumbnail = get_post_meta( $post->ID, $config->postMetaGameImg, true ) ) {
	 ...
	 * @return bool
	 */
	private static function _isPage() {
		return is_page();
	}

	private static function _isPost() {
		return is_single();
	}
	 ...
	 * @return string
	 */
	private static function _getTemplate( $list_items ) {
		return <<<MARKUP
