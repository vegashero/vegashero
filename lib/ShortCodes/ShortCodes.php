...
class ShortCodes {
	...
	public static function addShortCodes() {
		...
		add_shortcode( 'vh-game', array( self::class, 'renderSingleGame' ) );
		// tested
		add_shortcode( 'vh_game', array( self::class, 'renderSingleGame' ) );
		// tested
	}
	...
	public static function vh_table_func( $atts, $vhcontent = null ) {
		...
		extract(
			shortcode_atts(
				array(
					...
					// bonus column title
												'vh_devicehead' => '',
				// device compatibility column title
				),
				$atts
			)
		);
		...
		if ( $vh_bonushead == '' ) {
			$vh_bonushead = wp_strip_all_tags( __( 'Bonus', 'vegashero' ) ); }
		if ( $vh_devicehead == '' ) {
			$vh_devicehead = wp_strip_all_tags( __( 'Compatible Devices', 'vegashero' ) ); }
	...
	public static function vh_table_line_func( $atts ) {
		...
		if ( $vh_pc == '1' ) {
			$vh_pc = '<div class="results-desktop">Desktop</div>'; }
		if ( $vh_tablet == '1' ) {
			$vh_tablet = '<div class="results-tablet">Tablet</div>'; }
		if ( $vh_mobile == '1' ) {
			$vh_mobile = '<div class="results-mobile">Mobile</div>'; }
		if ( $vh_target == 'new' ) {
			$vh_target = '_blank';
		} else {
			$vh_target = '_self'; }
