...
class Vegashero_Import_Operator extends Vegashero_Import
{
    ...
    private function _insertNewGame($game, $operator) {
        ...
        $post_meta_game_img = add_post_meta($post_id, $this->_config->postMetaGameImg, sprintf("%s/%s/%s/cover.jpg", $this->_config->gameImageUrl, trim($operator), $game_title), true); // add post meta data
