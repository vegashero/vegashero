...
class Vegashero_Import_Provider extends Vegashero_Import
{
    ...
    private function _insertNewGame($game, $provider) {
        ...
        $post_meta_game_img = add_post_meta($post_id, $this->_config->postMetaGameImg, sprintf("%s/%s/%s/cover.jpg", $this->_config->gameImageUrl, trim($provider), $game_title), true); // add post meta data
