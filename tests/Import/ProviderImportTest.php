...
final class ProviderImportTest extends WP_UnitTestCase {
	...
	public function testReimportUpdatesGameType() {
		$flash_game       = \VegasHero\Helpers\Test::generateRandomGame(
			self::$faker,
			array(
				'status'   => 1,
				'type'     => 0,
		...
		$flash_posts      = \VegasHero\Helpers\Test::importGames( json_encode( array( $flash_game ) ), self::$importer, self::$config );
		$html5_game       = clone $flash_game;
		$html5_game->type = 1;
		$html5_posts      = \VegasHero\Helpers\Test::importGames( json_encode( array( $html5_game ) ), self::$importer, self::$config );
